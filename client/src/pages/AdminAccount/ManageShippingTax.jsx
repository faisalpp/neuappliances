...
import UpdateZone from '../../components/AdminDashboard/PopupForms/UpdateZone'
...
const ManageShippingTax = () => {
  ...
  const [updateZonePop,setUpdateZonePop] = useState(false);
  
  return (
    <>
    <CreateZone popup={createZonePop} setPopup={setCreateZonePop} />
    <UpdateZone popup={updateZonePop} setPopup={setUpdateZonePop} />
    <AdminAccount>
    ...
    <div className='flex flex-col space-y-5' >
      ...
      <div className="flex flex-col px-5 py-3 shadow-md rounded-md border-[1px] border-b6 w-full" >
        <div className='flex items-center' >
         ...
         <div className='flex space-x-2 justify-end w-full' ><button type="button" onClick={()=>setCreateZonePop(true)} className='self-end hover:shadow-md bg-b3 text-white text-xs px-3 rounded-2xl cursor-pointer py-2 font-bold' >Create&nbsp;Zone</button><Link to="/admin/manage-shipping-classes" className='self-end hover:shadow-md bg-b3 text-white text-xs px-3 rounded-2xl cursor-pointer py-2 font-bold' >Shipping&nbsp;Classes</Link></div>
        </div>
        <ShippingTable setEditor={setUpdateZonePop} />  
      </div>
      {/* Manage Tax */}
      <div>
