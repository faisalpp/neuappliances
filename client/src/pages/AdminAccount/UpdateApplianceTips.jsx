...
import {  updateTip,getTipBySlugWithCategories } from '../../api/admin/applianceTips'
...
const UpdateApplianceTip = () => {
  ...
  const blogCreationValidationSchema = Yup.object().shape({
    ...
    count: Yup.string().required('Tip Count is required'),
  });
  ...
  const GetBlogbySlug2 = async () => {
    ...
    const res = await getTipBySlugWithCategories(data)
    if(res.status === 200){
      ...
      setCount(res.data.blog.count)
      setKeywords(res.data.blog.metaKeywords)
      const filteredCategories = res.data.categories.filter(cat => cat.title.toLowerCase() !== category)
  ...
  const UpdateBlog2 = async (e) => {
    ...
    setSubmit(true)
    try{
     const data = {id,title,slug,thumbnail,tempImg,category,content,count}
    ...
    const res = await updateTip(formData);
    if(res.status === 200){
        ...
        <AdminAccount>
          ...
          <div className='flex w-full' >
           <div className='flex flex-col space-y-10 w-1/2' >
            ...
            <TextInput width="full" name="count" title="Tip Count" iscompulsory="true" type="text" value={count} onChange={(e)=>setCount(e.target.value)} error={errors && errors.includes('Tip Count is required') ? true : false} errormessage="Tip Count is required" placeholder="Tips Count" />
           </div>
...
export default UpdateApplianceTip