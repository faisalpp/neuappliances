                    ...
                    {data.map((section) => <>
                        {section.cardStyle === 'rating-card' ? <div>
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-8'>
                                {section.sectionItemsId.map((product) => <NavLink to={`/appliances/${category.toLowerCase()}/${section.type}/${product.rating}`} ><ProductCard key={product.title} title={product.title} image={`${import.meta.env.REACT_APP_INTERNAL_PATH}/storage/sectionItems/${product.image}`} rating={product.rating} /></NavLink>)}
                            </div>
                        </div> : null}
                        ...
                        {section.cardStyle === 'general-card' ? <div>
                            ...
                            <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-8'>
                                {section.sectionItemsId.map((product) => <NavLink to={`/appliances/${category.toLowerCase()}/${section.type}/${product.title.toLowerCase().replace(/\s/g, "-")}`} ><ProductCard key={product.title} title={product.title} image={`${import.meta.env.REACT_APP_INTERNAL_PATH}/storage/sectionItems/${product.image}`} /></NavLink>)}
                            </div>
                        </div> : null}
                        ...
                        {section.cardStyle === 'brand-card' ? <div>
                            ...
                            <div className='grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 mt-8'>
                                {section.sectionItemsId.map((product) => <NavLink to={`/appliances/${category.toLowerCase()}/${section.type}/${product.title.toLowerCase().replace(/\s/g, "-")}`} ><ProductCard key={product.title} brandname={product.title} brandimage={`${import.meta.env.REACT_APP_INTERNAL_PATH}/storage/sectionItems/${product.image}`} /></NavLink>)}
                            </div>
                        </div> : null}
                        ...
                        {section.cardStyle === 'color-card' ? <div>
                            <RelatedProducts title={section.title} />
                            <div className='grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 mt-8'>
                                {section.sectionItemsId.map((product) => <NavLink to={`/appliances/${category.toLowerCase()}/${section.type}/${product.title.toLowerCase().replace(/\s/g, "-")}`} ><ProductCard key={product.title} colorname={product.title} colorimage={`${import.meta.env.REACT_APP_INTERNAL_PATH}/storage/sectionItems/${product.image}`} /></NavLink>)}
                            </div>
                        </div> : null}
                    </>
                    )}
