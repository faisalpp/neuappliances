    ...
    <div className="inline-block min-w-full py-2 sm:px-6 lg:px-8">
      <div className="overflow-hidden">
        <table className="min-w-full text-center text-sm font-light">
          ...
          <tbody>
            {data.map((item)=><ProductRow img={process.env.REACT_APP_DEV ? `${process.env.REACT_APP_INTERNAL_PATH}/${item.images[0]}`:`${item.images[0]}`} title={item.title} salePrice={item.salePrice} regularPrice={item.regularPrice} rating={item.rating} />)}
          </tbody>
        </table>
      </div>
    </div>
