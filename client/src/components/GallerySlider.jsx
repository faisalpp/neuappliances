  ...
  return (
    ...
    <div className='relative my-8 mx-5' >
     ...
     <div id="id6" className='flex lg:overflow-x-hidden overflow-x-scroll xl:space-x-[9px] space-x-4 lg:space-x-3 scroll-smooth' >
      {media.length > 0 ? media.map((item,index)=><img key={index} onClick={()=>setImg(item.imageUrl)} src={item.imageUrl} className={`xl:h-[142px] xl:w-[171px] cursor-pointer lg:h-[100px] lg:w-[130px] w-36 h-32 rounded-2xl ${img === item.imageUrl ? 'border-2 border-white rounded-xl' : 'border-none'}  `} />):null}
     </div>
    </div>
