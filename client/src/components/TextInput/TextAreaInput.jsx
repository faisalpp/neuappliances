...
const TextAreaInput = (props) => {
  ...
  <div className={`flex flex-col space-y-1 ${props.width === 'full' ? 'w-full' : 'w-1/2'}`}>
    ...
    <textarea {...props} className={`text-sm outline-none border-[1px] ${props.error?'border-red-500':'border-gray-200'} w-full px-2 py-3 rounded-md ${props.height?props.height:''} overflow-x-hidden overflow-y-scroll`}></textarea>
  </div>
  )
}
