  ...
  return (
     ...
     <div className='w-1/2' >
           ...
           <select {...props} onChange={props.change} className={`border ${props.error ? 'border-red-500' :'border-[rgba(0,0,0,0.16)]'} rounded-lg h-10 text-sm px-4 w-full outline-none appearance-none`}>
            {options && options.length>0 ? options.map((option,index)=><option key={index} value={props.name === 'category'? option._id: props.name !== 'category' && option.title? option.title.toLowerCase().replace(' ','-'): option.toLowerCase().replace(' ','-')} >{option.title ? option.title:option}</option>):<option>{props.title} Not Found!</option>}
           </select>
