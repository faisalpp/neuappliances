            ...
            <div className='py-10 lg:py-16 xl:py-20 w-full max-w-1680px px-4 sm:px-10 lg:px-16 xl:px-20 2xl:px-120px mx-auto flex maxlg:flex-col gap-10 lg:gap-7 xl:gap-10'>
                <div className="tab-buttons maxlg:order-2 lg:w-[50%] flex flex-col gap-2">
                    {tabsData.map((tab) => (
                        <button
                            key={tab.id}
                            className={`p-5 xl:p-6 xl:text-lg font-semibold flex justify-between items-center  text-left border border-[rgba(0,0,0,0.15)] rounded-2xl ${activeTab === tab.id ? 'active text-white bg-b7' : 'text-gray-600'}`}
                    ...
                    {tabsData.map((tab) => (
                        activeTab === tab.id && <div key={tab.id} className='flex flex-col gap-3 sm:gap-4'>
                            {tab.content.map((content, index) => (
                                <FaqAccordion title={content.title} parent='gap-3 bg-b8/60 [&>div>h6]:maxmd:text-sm text-white p-4 md:px-8 md:py-6 rounded-xl border-none text-black h-auto' icon='text-xl text-black' textStyle='font-bold text-md text-black' child='[&>p]:text-sm text-black font-normal' answer={content.answer} />
                            ))}
                            {/* {tab.content} */}
                        </div>
                    ))}
