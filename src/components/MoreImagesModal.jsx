...
const MoreImagesModal = ({state,setState,images}) => {
  ...
  const [image,setImage] = useState(images ? `${process.env.REACT_APP_INTERNAL_PATH}/${images[0]}`:'')
     ...
     <div className='relative flex justify-center w-10/12'>
      ...
      <div className='flex flex-col bg-white w-full py-8 rounded-xl' >
        ...
        <div className='flex justify-center mt-8 items-center space-x-5 w-full' >
          {images && images.map((img)=><div className='border-[1px] border-b3 rounded-md px-1 py-1 cursor-pointer' ><img onClick={()=>setImage(`${process.env.REACT_APP_INTERNAL_PATH}/${img}`)} src={`${process.env.REACT_APP_INTERNAL_PATH}/${img}`} alt='' className='w-12' /></div>)}
        </div>
      </div>
    </div>
    </div>
